...
param_dir = work_dir / "params_uniform"
...
def generate_sobol_sequence(num_files, seed):
    ...
    for name in names_for_problem:
        ...
        lower = norm.ppf(0.025, loc=mean, scale=sd)
        upper = norm.ppf(0.975, loc=mean, scale=sd)
        if name == 'gap':
            lower = max(lower, min_gap) # ensure that the lower bound for the gap is at least the minimum gap
        if name == 'solar_transmittance':
            lower = max(lower, 0) # ensure that the lower bound for solar transmittance is at least 0
            upper = min(upper, max_solar_transmittance) # ensure that the upper bound for solar transmittance is at most the maximum solar transmittance
...
# mpirun -hostfile myhosts -np 225 /jumbo/keller-lab/Applications/mambaforge/envs/eplus/bin/python 4a_eplus_uniformsampling.py

...
from scipy.stats import norm, qmc, beta, uniform
import SALib
...
base_output_idf_dir = work_dir / "randomized_idfs_beta"
param_dir = work_dir / "params_beta"
...
def generate_sobol_sequence(num_files, seed):
    ...
    problem = {
        ...
        'bounds': [[0,1]] * len(names_for_problem),
    }

    # conduct sobol sequence sampling
    N = 1000 # baseline number of samples
    uniform_sampling = sobol.sample(problem, N, calc_second_order=False, seed = seed) # array with dimensions [N*(P+2), P]

    # beta transformation
    alpha_value = 2
    beta_value = 2
    param_values = np.zeros_like(uniform_sampling) # initialize array to store transformed parameter values

    for j, name in enumerate(names_for_problem):
        mu = means_for_problem[name]
        sigma = std_for_problem[name]

        # your physical bounds
        lower = norm.ppf(0.025, loc=mu, scale=sigma)
        upper = norm.ppf(0.975, loc=mu, scale=sigma)

        if name == 'gap':
            lower = max(lower, min_gap)

        if name == 'solar_transmittance':
            lower = max(lower, 0)
            upper = min(upper, max_solar_transmittance)

        # inverse-CDF transform
        x = beta.ppf(uniform_sampling[:, j], alpha_value, beta_value)

        # scale from [0,1] to [lower, upper]
        param_values[:, j] = lower + x * (upper - lower)
...
# mpirun -hostfile myhosts -np 225 /jumbo/keller-lab/Applications/mambaforge/envs/eplus/bin/python 4b_eplus_betasampling.py
